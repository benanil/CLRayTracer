...
int Renderer::Initialize()
{
	...
	// initialize openCL
	{
		...
		cl::Program program(context, "../karnels/karnel_main.cl");
		
		// specify which kernel from the program to execute
		kernels[0] = cl::Kernel(program, "hello");
		...
		clerr |= kernels[0].SetArg(0, &input);
		clerr |= kernels[0].SetArg(1, &startVal);
		size_t global = DATA_SIZE;

		// enqueue the kernel command for execution
		clEnqueueNDRangeKernel(command_queue, kernels[0], 1, NULL, &global,
			NULL, 0, NULL, NULL);
	...
	// unsigned* texData = new unsigned[801 * 601];
	// std::fill(texData, texData + (801 * 601), 0xffffffffu);
	// textureLoc = glGetUniformLocation(shaderProgram, "texture0");
	// glActiveTexture(GL_TEXTURE0);
	// glGenTextures(1, &screenTexture);
	// glBindTexture(GL_TEXTURE_2D, screenTexture);
	// glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, Window::GetWidth(), Window::GetHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE, texData);
	// delete[] texData;
