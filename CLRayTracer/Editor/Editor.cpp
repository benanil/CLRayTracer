...
void Editor::AddOnEditor(void(*action)() action) {}
void Editor::Begin() { }
void Editor::Render(unsigned screenRenderImageGl) { }
void Editor::DeepDark() { }
void Editor::Light() { }
...
static void(*OnEditor[40])();
static int numOnEditor = 0;

void Editor::AddOnEditor(void(*action)()) {
	OnEditor[numOnEditor++] = action;
}

static ImVec2 WindowScale, PanelPosition;
static ImVec2 newScale, newPosition;

static void ChangeScale()
{
    PanelPosition = newPosition;
    WindowScale = newScale;
    Renderer::OnWindowResize((int)newScale.x, (int)newScale.y);
}
...
static void DrawRendererWindow(unsigned texture)
{
    ...
    if (!first && WindowScale.x != newScale.x || WindowScale.y != newScale.y ||
        PanelPosition.x != newPosition.x || PanelPosition.y != newPosition.y)
    {
        Engine_AddEndOfFrameEvent(ChangeScale);
    }

    if (!first) {
        ImGui::Image((void*)texture, WindowScale, { 0.0f,1.0f }, {1.0f,0.0f});
...
void Editor::Light()
{
    ...
    for (int i = 0; i < ImGuiCol_COUNT; ++i)
    {
        style.Colors[i].x = 1.0f - style.Colors[i].x;
        style.Colors[i].y = 1.0f - style.Colors[i].y;
        style.Colors[i].z = 1.0f - style.Colors[i].z;
        style.Colors[i].w = 1.0f - style.Colors[i].w;
    }
}
